...
export default function Base() {
  ...
  return (
    <motion.div
      className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white px-6 py-4"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        <h2 className="text-4xl font-bold text-purple-300 mb-4">Welcome to QA Skills</h2>
      ...
      >
        {[
          ...
          <motion.div
            key={index}
            className="bg-gray-700 shadow-lg rounded-xl p-6 text-center"
            variants={{
      ...
      >
        {stats.map((stat, idx) => (
          <div
            key={idx}
            className="bg-gray-800 rounded-xl text-center p-6 border border-purple-600"
          >
      ...
      <div className="mt-20 max-w-4xl mx-auto">
        <h2 className="text-2xl font-bold text-purple-300 text-center mb-8">What Students & Teachers Say</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {comments.map((c, i) => (
            <motion.div
              key={i}
              className="bg-gradient-to-r from-purple-900 to-purple-700 p-5 rounded-xl shadow"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 1.2 + i * 0.2 }}
            >
              <p className="italic text-white mb-3">"{c.text}"</p>
              <p className="text-right text-purple-100 font-medium">{c.user}</p>
            </motion.div>
          ))}
        </div>
      </div>
